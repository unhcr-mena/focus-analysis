...
#opreferencemena <- opreferencemena[ (1:43),]
...
for (i in 1:nrow(opreference))
{
  ...
  z <-
    getNodeSet(
      plancountryparse,
      "//ppgs/PPG/goals/Goal/rightsGroups/RightsGroup/problemObjectives/ProblemObjective/indicators/Indicator"
    )
  n <- length(z)
  ...
  for (i in 1:nrow(impindicatortemp))
  {
    if (is.na(impindicatortemp[i, c("Mid.Year2")])) {
      impindicatortemp[i, c("Mid.Year")] <-
        as.numeric(substr(impindicatortemp[i, c("Mid.Year1")] , 1, (
          regexpr(
            ";",
            impindicatortemp[i, c("Mid.Year1")] ,
            ignore.case = FALSE,
            fixed = TRUE
          )
        ) - 1))
      ...
      impindicatortemp[i, c("Mid.Year")] <-
        impindicatortemp[i, c("Mid.Year2")]
    }
    if (is.na(impindicatortemp[i, c("Year.End2")])) {
      impindicatortemp[i, c("Year.End")] <-
        as.numeric(substr(impindicatortemp[i, c("Year.End1")] , 1, (
          regexpr(
            ";",
            impindicatortemp[i, c("Year.End1")] ,
            ignore.case = FALSE,
            fixed = TRUE
          )
        ) - 1))
      ...
      impindicatortemp[i, c("Year.End")] <-
        impindicatortemp[i, c("Year.End2")]
    }
  }
  ...
  getPPGContent =
    function(x)
    {
      ...
      indicator = xpathSApply(
        x,
        "./goals/Goal/rightsGroups/RightsGroup/problemObjectives/ProblemObjective/indicators/Indicator",
        xmlGetAttr,
        'ID'
      )
      cbind(
  ...
  temp <-
    xpathApply(plancountryparse, "//ppgs/PPG", getPPGContent)
  #str(temp)
  # as.data.frame(temp)
  impindicatorobj <- as.data.frame(do.call("rbind", temp))

  impindicatortemp1 <-
    merge (x = impindicatortemp ,
           y = impindicatorobj,
           by = "indicatorid",
           all.x = TRUE)
...
data$end2base <-
  ifelse(data$Baseline == 0,
         0,
         (data$Year.End - data$Baseline) / data$Baseline)
#hist(data$end2base)
data$end2base.sit <-
  as.factor(findCols(
    classIntervals(
...
data$mid2base <-
  ifelse(data$Baseline == 0,
         0,
         (data$Mid.Year - data$Baseline) / data$Baseline)
#hist(data$mid2base)
data$mid2base.sit <-
  as.factor(findCols(
    classIntervals(
...
data$mid2net <-
  ifelse((data$OL.Target - data$Baseline) == 0,
         0,
         (data$Mid.Year - data$Baseline) / (data$OL.Target - data$Baseline)
  )
data$mid2net.sit <-
  as.factor(findCols(
    classIntervals(
...
data$end2net <-
  ifelse((data$OL.Target - data$Baseline) == 0,
         0,
         (data$Year.End - data$Baseline) / (data$OL.Target - data$Baseline)
  )
data$end2net.sit <-
  as.factor(findCols(
    classIntervals(
...
data.impact$idrecord <-
  paste(data.impact$operationName,
        data.impact$Goal,
        data.impact$Population.Group,
        sep = "-")

#names(data.impact)
write.csv(data.impact, "data/impact.csv", row.names = FALSE)
rm( data, SituationCode,framework, opreference, getPPGContent)
