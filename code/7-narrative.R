...
opreference <- opreferencemena.narrative[opreferencemena.narrative$planningPeriod %in% c("2016","2017","2018","2019","2020") , c( "operationID",    "attr" ,"planid" ,"planname", "planningPeriod",
...
for (i in 1:nrow(opreference))
{
  ...
  if (n1 == 0) { print(paste(i , "There's no narrative linked to this objective for the report ", collapse = NULL) )
...
data.narrative2$idrecord <- paste(data.narrative2$operationName,data.narrative2$Goal,
                                  data.narrative2$Population.Group, sep = "-")
